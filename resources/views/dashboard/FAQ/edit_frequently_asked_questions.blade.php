...
<div style="width: 70%">
    ...
    @endif
    <form action="{{ url('dashboard/FAQ/editFAQ') }}" method="POST" enctype="multipart/form-data" id="myForm">
        @csrf
        @foreach ($data as $item)
            <div><input type="text" hidden name="id" id="" value="{{ $item->id }}"></div>
            <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label">Question:</label>
                <input type="text" class="form-control" name="question" value="{{ $item->question }}" id="question">
            </div>
            <div class="mb-3">
                <label for="exampleInputPassword1" class="form-label">Answer:</label>
                <input type="text" class="form-control" name="reply" value="{{ $item->answer }}" id="question">
            </div>
        @endforeach
